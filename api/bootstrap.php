...
function allowed_types(){ return ['settings','news','events','results','documents','people','gallery','partners','social','pages']; }
...
function item_key($item){ return $item['route'] ?? $item['slug'] ?? $item['id'] ?? $item['title'] ?? $item['name'] ?? $item['date'] ?? ''; }
function normalize_page_item($item){ if(!is_array($item)) return $item; $route=trim($item['route'] ?? ($item['slug'] ?? ''), '/'); $slug=trim($item['slug'] ?? $route, '/'); $item['route']=$route; $item['slug']=$slug; if(empty($item['updated'])) $item['updated']=date('Y-m-d'); return $item; }
?>
